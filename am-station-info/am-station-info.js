...
(() => {
    ...
    const DYNAMIC_PROFILE_MODEL =[
        // Natt: D-laget forsvinner (LUF ~0), F-laget svekkes (MUF ~11). Optimalt for MW/lav SW.
        { event: 'nadir',      offsetHours: 0,    luf: 0.1, greenStart: 0.5, greenEnd: 7.0,  muf: 11.0 },
        // Soloppgang: D-laget begynner å bygge seg opp, F-laget lades.
        { event: 'sunrise',    offsetHours: -1.0, luf: 0.2, greenStart: 1.0, greenEnd: 9.0,  muf: 14.0 },
        { event: 'sunrise',    offsetHours: 1.5,  luf: 3.0, greenStart: 6.0, greenEnd: 16.0, muf: 20.0 },
        // Midt på dagen: Max D-lag absorpsjon (Høy LUF), Max F-lag (Høy MUF). Optimalt for høy SW.
        { event: 'solarNoon',  offsetHours: 0,    luf: 6.0, greenStart: 12.0, greenEnd: 22.0, muf: 28.0 },
        // Solnedgang: D-laget forsvinner raskt, F-laget holder litt på ladningen.
        { event: 'sunset',     offsetHours: -1.0, luf: 3.0, greenStart: 7.0, greenEnd: 18.0, muf: 22.0 },
        { event: 'sunset',     offsetHours: 1.5,  luf: 0.5, greenStart: 1.5, greenEnd: 10.0, muf: 14.0 }
    ];
    ...
    class SettingsModal {
            ...
            <div id="${this.overlayId}" class="am-native-modal" style="display:none;">
                <div class="am-native-content">
                    ...
                    <div class="am-native-header">
                        ...
                        <a id="am-native-ap-btn" class="am-native-ap" href="/AM-Station_info/admin" target="_blank" title="Go to Admin Panel" style="display:none;">AP</a>
                    ...
                    <div class="am-native-body">
                        <fieldset class="am-fieldset">
                            ...
                            <div class="am-row">
                                <label>Freq Margin (+/-)</label>
                                <div style="display: flex; align-items: center; gap: 10px;">
                                    <input type="range" id="am-set-margin" min="0" max="3" step="1" style="width: 110px; cursor: pointer;">
                                    <span id="am-set-margin-val" style="width: 40px; text-align: right; font-weight: bold; font-size: 14px; color: var(--color-main-bright);">0 kHz</span>
                                </div>
                            </div>
                            <div class="am-row">
        ...
        open(isAdmin) {
            // Vis AP-knapp om admin
            if (isAdmin) $('#am-native-ap-btn').show();
            else $('#am-native-ap-btn').hide();

            $('#am-set-list').val(this.settings.get('selectedList'));
            $('#am-set-margin').val(this.settings.get('freqMargin'));
            $('#am-set-margin-val').text(this.settings.get('freqMargin') + ' kHz');
            $('#am-set-margin').off('input').on('input', function() {
                $('#am-set-margin-val').text($(this).val() + ' kHz');
            });
    ...
    class AmStationPlugin {
        ...
        init() {
            ...
            $.getJSON('/AM-Station_info/api/auth-check').done(data => { 
                this.isAdmin = data.isAdmin; 
                if (data.config) {
                    CONFIG.maxFreq = data.config.maxFreq;
                    Object.keys(this.settings.defaults).forEach(key => {
                        if (data.config[key] !== undefined) {
                            this.settings.defaults[key] = data.config[key];
                            
                            const strictAdminOverrides =[
                                'useMapModal', 
                                'useSettingsModal', 
                                'useServerPos', 
                                'sortType'
                            ...
                            if (strictAdminOverrides.includes(key) || data.config.useSettingsModal === false) {
                                this.settings.set(key, data.config[key]);
                            } 

                            else if (data.config[key] === false) {
                                delete this.settings._settings[key];
                                this.settings._saveSettings();
                            }
                        }
                    });

                    if (!this.settings.get('useSettingsModal') && !this.isAdmin) {
                        $('#am-settings-gear').hide();
                    } else {
                        $('#am-settings-gear').show();
                    }
                }
            }).fail(() => { this.isAdmin = false; });
        ...
        _injectCombinedStyles() {
            ...
            .am-native-ap { position: absolute; top: 0; right: 42px; width: 32px; height: 32px; background-color: var(--color-4); border: 1px solid var(--color-5); border-radius: 5px; cursor: pointer; font-size: 14px; font-weight: bold; line-height: 28px; text-align: center; color: var(--color-main); transition: 0.3s ease; text-decoration: none; display:none; }
            .am-native-ap:hover { background-color: var(--color-5); color: var(--color-1); }
            ...
            #aoki-kp-group { display: flex; gap: 8px; align-items: center; display: none; }
            #aoki-map-kp { font-size: 13px; font-weight: bold; padding: 3px 8px; border-radius: 5px; background-color: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.15); display: none; }
            #aoki-map-kp-forecast { font-size: 13px; font-weight: bold; padding: 3px 8px; border-radius: 5px; background-color: rgba(0,0,0,0.3); border: 1px dashed rgba(255,255,255,0.25); display: none; }
            ...
            /* --- MOBILVISNING TILPASNINGER --- */
            @media (max-width: 700px) { 
                /* 1. Kartet */
                #aoki-map-modal .aoki-map-dialog { box-sizing: border-box; top: 10px; right: 10px; bottom: 10px; left: 10px; width: calc(100vw - 20px); height: 75vh; max-height: none; transform: none; } 
                #aoki-map-infobox { display: none !important; } 
                .footer-data-left, .footer-data-right { display: none !important; }
                .graph-labels, .marker.label::after { display: none !important; }
                .aoki-map-title { font-size: 16px; } 
                .aoki-map-close { width: 65px; font-size: 14px; } 
                #aoki-map-kp, #aoki-map-kp-forecast { font-size: 11px; padding: 2px 4px; }

                /* 2. Hovedpanelet (RDS-erstatningen) på hovedsiden */
                body.am-station-info-active #aoki-plugin-display {
                ...
                /* Skjul Favoritt- og Lås-knapp på mobil, samt Tooltip */
                body.am-station-info-active #aoki-fav-btn,
                body.am-station-info-active #aoki-lock-btn,
                #aoki-station-info-tooltip { 
                    display: none !important; 
                }
                
                /* LINJE 1: Stasjonsnavn */
                body.am-station-info-active .station-name { 
                    font-size: 1.35em; 
                    margin-top: 0px; 
                    margin-bottom: 3px; 
                }
                
                /* LINJE 2: Sted og Land */
                body.am-station-info-active .station-location { 
                    font-size: 0.90em; 
                    margin-top: 0px; 
                    margin-bottom: 4px; 
                    line-height: 1.2;
                ...
                /* LINJE 3: Språk, Tid og Effekt */
                body.am-station-info-active .station-meta { 
                    font-size: 0.90em; 
                    margin-top: 0px; 
                    margin-bottom: 0px; 
                    line-height: 1.2;
                ...
                /* Navigasjons-knapper (</>) og Kilde (MWLIST etc) nederst */
                body.am-station-info-active #aoki-nav-controls { 
                    bottom: 5px !important; 
                }
                body.am-station-info-active #aoki-nav-controls button.nav-btn { 
                    width: 50px !important; /* Gjør navigasjonsknappene bredere */
                }
                body.am-station-info-active #aoki-source-display { 
                    bottom: 5px !important; 
                }

                /* Tannhjulet: Flytt til bunn høyre, gjør permanent synlig og litt større */
                body.am-station-info-active #am-settings-gear {
                    top: auto !important;
                    bottom: 5px !important;
                    right: 5px !important;
                    opacity: 0.7 !important; /* Alltid synlig (ingen hover-krav) */
        ...
        _buildUI() {
            ...
            // Hvis Modal er deaktivert, send admin rett til panelet.
            settingsBtn.click((e) => { 
                e.stopPropagation(); 
                if (this.isAdmin && !this.settings.get('useSettingsModal')) {
                    window.open('/AM-Station_info/admin', '_blank');
                } else {
                    this.modal.open(this.isAdmin); 
                }
            });
            ...
            display.click((e) => {
                if (!$(e.target).closest('button, #am-settings-gear').length) {
                    ...
                    // Åpner map kun hvis Map Modal er aktivert i innstillingene
                    if (this.settings.get('useMapModal') && st) {
                        this._openMap(st);
                    }
                    this._hideTooltip();
                }
        ...
        _showTooltip() {
            // Hvis Map Modal er deaktivert, ikke vis oppfordring til å klikke.
            if (!this.settings.get('useMapModal')) {
                this.elements.tooltip.html('This panel only shows information about available<br>stations in the database, this is not RDS data.');
            } else {
                this.elements.tooltip.html('This panel only shows information about available<br>stations in the database, this is not RDS data.<br><br>Click to open local map.');
            }
                ...
                <div class="aoki-map-dialog">
                  <div class="aoki-map-header">
                    <div class="header-left-group">
                        ...
                        <div id="aoki-kp-group">
                            <span id="aoki-map-kp" title="Global Kp Index">Kp: -</span>
                            <span id="aoki-map-kp-forecast" title="Expected Kp next 3 hours">Next 3H: -</span>
                        </div>
                    </div>
                  ...
                  <div class="aoki-map-footer">
                    ...
                    <div id="propagation-graph-container">
                      ...
                      <div id="propagation-graph" style="width: 100%; height: 20px; border-radius: 5px; border: 1px solid rgba(255,255,255,0.2); position: absolute; bottom: 0; box-shadow: inset 0 2px 5px rgba(0,0,0,0.5);">
                        <div id="frequency-line" style="position: absolute; height: 100%; width: 2px; z-index: 10; display: none; box-shadow: 0 0 4px #000;"></div>
                      </div>
        ...
        _renderStation() {
            ...
            const useServerPos = this.settings.get('useServerPos');
            
            if (this.isLocked) this.lockedStationName = station.name;

            let distSpan = '';
            if (useServerPos && station.distance !== null) {
                const dText = useMiles ? `${Math.round(station.distance * 0.621371)} mi` : `${Math.round(station.distance)} km`;
                distSpan = ` <span class="text-gray">▪</span> ${dText}`;
            }
            ...
            const html = `
                ...
                <div class="station-location">${loc} <span class="text-gray">[${station.country}]</span>${distSpan}</div>
            ...
            if (this.isAdmin && isMWLW) {
                ...
                const isFav = this.settings.favorites.some(f => f.name === station.name && f.frequency === currentStationFreq);
                if (isFav) {
            ...
            if (this.settings.get('useMapModal')) {
                this.elements.display.css('cursor', 'pointer');
            } else {
                this.elements.display.css('cursor', 'default');
            }
        }
        ...
        _toggleFavorite() {
            ...
            const isFav = this.settings.favorites.some(f => f.name === st.name && f.frequency === fKHz);
            ...
            $.ajax({
                ...
                if (data.success) {
                    this.settings.favorites = data.favorites;
                    this._renderStation(); 
        ...
        async _openMap(station) {
            ...
            if (this.settings.get('useServerPos')) {
            ...
            if (!isNaN(stLat) && !isNaN(stLon)) {
                ...
                if (this.settings.get('showPath') && this.settings.get('useServerPos')) {
            ...
            const useMiles = this.settings.get('useMiles');
            let distText = 'N/A';
            if (this.settings.get('useServerPos') && station.distance != null) {
            ...
            const infoHTML = `
              <div id="aoki-map-infobox">
                ...
                <div class="info-content" style="${this.settings.get('extendedInfo') ? 'display:block' : ''}">
                  ...
                  ${this.settings.get('useServerPos') ? `<p><strong>Distance:</strong> ${distText}</p>` : ''}
        ...
        _fetchKpIndex() {
            ...
            const $fc = $('#aoki-map-kp-forecast');
            
            $kp.text('KP: ...').css('color', '#ccc').show();
            $fc.hide();
            
            $.getJSON('/AM-Station_info/api/kp-index').done(res => {
                if (!res || !res.success || res.current === null || res.current === undefined) {
                    $kp.text('KP: Err').css('color', '#e74c3c');
                    return;
                }
                
                const val = parseFloat(res.current);
                ...
                if (this.settings.get('showKpForecast') && res.forecast !== null && res.forecast !== undefined) {
                    const fcVal = parseFloat(res.forecast);
                    $fc.text(`Next 3H: ${fcVal.toFixed(2)}`).show();
                    
                    if (fcVal < 4) $fc.css('color', '#2ecc71');
                    else if (fcVal < 5) $fc.css('color', '#f1c40f');
                    else $fc.css('color', '#e74c3c');
                }
                
            }).fail(() => $kp.text('KP: Err').css('color', '#e74c3c'));
        }
        ...
        _getDynamicProfile() {
            ...
            // Returnerer nå rene MHz-verdier direkte
            ...
            return { luf, muf, gs, ge };
        }

        _updatePropagationGraph(profile) {
            const maxFreq = CONFIG.maxFreq || 27;
            
            // Hjelpefunksjon for å gjøre MHz om til % for CSS-gradienten
            const toPct = (mhz) => Math.max(0, Math.min(100, (mhz / maxFreq) * 100));
            
            const pLuf = toPct(profile.luf);
            const pGs = toPct(profile.gs);
            const pGe = toPct(profile.ge);
            const pMuf = toPct(profile.muf);
            
            // Bygger en sømløs farge-gradient (Mørkerød -> Oransje -> Grønn -> Oransje -> Mørkerød)
            const gradient = `linear-gradient(to right, 
                rgba(139, 0, 0, 0.95) 0%, 
                rgba(139, 0, 0, 0.85) ${pLuf}%, 
                rgba(211, 84, 0, 0.9) ${pLuf + (pGs - pLuf) / 2}%, 
                rgba(39, 174, 96, 0.9) ${pGs}%, 
                rgba(39, 174, 96, 0.9) ${pGe}%, 
                rgba(211, 84, 0, 0.9) ${pGe + (pMuf - pGe) / 2}%, 
                rgba(139, 0, 0, 0.85) ${pMuf}%, 
                rgba(139, 0, 0, 0.95) 100%
            )`;
            
            $('#propagation-graph').css('background', gradient);
        }

        _updateFrequencyMarker(freq, profile) {
            const fMhz = freq / 1000;
            const maxFreq = CONFIG.maxFreq || 27;
            const pct = (fMhz / maxFreq) * 100;
            
            $('#frequency-pointer-top').css('left', pct+'%').show();
            $('#frequency-pointer-bottom').css('left', pct+'%').show();
            $('#frequency-line').css('left', pct+'%').show();
            
            // Farge på selve frekvens-nålen basert på hvor den befinner seg
            let c = '#c0392b'; // Rød (Død sone)
            if (fMhz >= profile.gs && fMhz <= profile.ge) {
                c = '#2ecc71'; // Grønn (Optimal)
            } else if (fMhz > profile.luf && fMhz < profile.muf) {
                c = '#f39c12'; // Oransje (Marginal)
            }
