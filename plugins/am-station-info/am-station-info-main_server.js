...
try {
    ...
    for (const file of stationFiles) {
        ...
        // Slår sammen listene uten noen sjekk
...
endpointsRouter.get('/aoki-api-proxy', (req, res) => {
    ...
    const resultat = filtrerteStasjoner.map(stasjon => {
        ...
        return {
            name: stasjon.name, location: stasjon.location, country: stasjon.country,
            language: stasjon.language, timeUTC: stasjon.time_utc, power: stasjon.power_kw,
            distance: avstand, 
...
logInfo(`${pluginName}: AM-Station-Info endpoint initialized (v1.0 - Final Source Logic).`);